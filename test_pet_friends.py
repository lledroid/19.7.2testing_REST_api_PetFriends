...
def test_add_pets_with_valid_data(name='Mysya', animal_type='cat', age='1'):
    ...
    _, api_key = pf.get_app_key(valid_email, valid_password)
    status, result = pf.add_new_pets(api_key, name, animal_type, age)
    assert status == 200
...
def test_add_pet_negative_age_number(name='Mysya', animal_type='cat', age='-1'):
    ...
    Тест не будет пройден если питомец будет добавлен на сайт с отрицательным числом в поле возраст.
     '''
 pf.get_app_key(valid_email, valid_password)
    _, result = pf.add_new_pets(api_key, na  

    _, api_key = me, animal_type, age, pet_photo)

    assert age not in result['age'], 'Питомец добавлен на сайт с отрицательным числом в поле возраст'

def test_add_pet_with_four_digit_age_number(name='Mysya', animal_type='cat', age='-1'):
